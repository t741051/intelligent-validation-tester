  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-gray-500">所有待驗證的 DUT 設備及其連接狀態</p>
      </CardHeader>
      <CardContent className="p-0">
        {isLoading ? (
          <div className="p-6 text-gray-400">載入中…</div>
        ) : duts.length === 0 ? (
          <div className="p-12 text-center text-gray-400">尚未新增 DUT</div>
        ) : (
                ...
                {duts.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-600">{d.data_format || "-"}</td>
                      <td className={`px-4 py-3 text-sm ${responseTimeClass(d.response_time_ms)}`}>
                        {d.response_time_ms != null ? `${d.response_time_ms}ms` : "-"}
                      </td>
                      <td className="px-4 py-3 text-gray-600 text-xs">{formatDate(d.last_check)}</td>
